...
class ShoppingController extends PublicController {
	...
	public function index(){
        ...
        foreach ($cart as $k => $v) {
        	...
        	$cartsss['uid']  = $v['uid'];
        	$cartsss['pid']  = $v['pid'];
        	$cartsss['price']  = $v['price'];
        	$cartsss['num']  = $v['num'];
        	$pro_info = $product->where('id='.intval($v['pid']))->field('name,photo_x')->find();
        	$cartsss['pro_name']=mb_strimwidth($pro_info['name'], 0, 30, '...','UTF-8');
        	$cartsss['photo_x']=__DATAURL__.$pro_info['photo_x'];
        	$carts[] = $cartsss;
        }

		echo json_encode(array('status'=>1,'cart'=>$carts));
		exit();
    }
    ...
    */
    ...
    public function deal_str($str){
        return mb_strimwidth($str, 0, 35, '...');
    }

    public function cut_str($str,$cutleng)
    {
    $str = $str; //要截取的字符串
