...
class ShangchangController extends PublicController{
	...
	public function setlogininfo(){
		...
		//检测用户名是否已存在
		$check = M('shopuser')->where('del=0 AND name="'.$username.'"')->getField('id');
		if (intval($check)) {
			$this->error('用户名已存在.');
			exit;
		}

		$checkinfo = intval(M('shopuser')->where('shop_id='.intval($shop_id).' AND del=0')->getField('id'));
		$data = array();
		$data['name'] = $username;
		$data['pwd'] = md5(md5($password));
		$data['uname'] = M('shangchang')->where('id='.intval($shop_id))->getField('name');
		if ($checkinfo) {
			$res = M('shopuser')->where('id='.intval($checkinfo))->save($data);
		}else{
			$data['shop_id'] = $shop_id;
			$data['addtime'] = time();
			$res = M('shopuser')->add($data);
		}
